...
def test_root_operators_mirror_is_gone():
    root_operators = Path(__file__).resolve().parents[1] / "operators"

    assert not root_operators.exists()
...
def test_root_gremlin_payload_strays_are_gone():
    ...
    assert not (repo_root / "jl_engine_core" / "data" / "operators" / "The_Gremlin_Full.json").exists()
