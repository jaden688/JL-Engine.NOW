...
def test_web_ui_shell_script_targets_switchboard_routes() -> None:
    ...
    assert "/quest/operators/mpf" not in script
...
def test_legacy_operators_api_alias_matches_agents_profiles() -> None:
    client = TestClient(api_main.app)

    profiles_response = client.get("/quest/agents/profiles/mpf")
    operators_response = client.get("/quest/operators/mpf")

    assert profiles_response.status_code == 200
    assert operators_response.status_code == 200
    assert operators_response.json()["operators"] == profiles_response.json()["agent_profiles"]
