...
- **Fat-agent runtime** — full operator capsules with behavior rules, memory, cognitive gears, and backend routing loaded per session
- **Modular agents** — composable shells that expand at load time into a resolved operator payload
...
- `game_integrations/`: game operator integration layer
...
JL Engine does not treat fat agents as a sidecar. The engine loads them as the active operator payload for a session.
