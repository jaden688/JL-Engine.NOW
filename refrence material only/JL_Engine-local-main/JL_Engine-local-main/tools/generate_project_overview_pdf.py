...
def cover_slide(ax):
    ...
    ax.text(
        0.08,
        0.75,
        "Project overview for the local-first engine, operator runtime, and operator stack",
        color=MUTED,
        fontsize=15,
        ha="left",
        va="top",
...
def repo_map_slide(ax):
    ...
    footer(ax, "The runtime favors data files over hard-coded operators, which makes the system easy to extend.")
...
def build_slides():
    return [
        Slide("JL Engine Local", "Project overview for the local-first engine, operator runtime, and operator stack.", cover_slide),
        Slide("What this repository is", "A local-first runtime for agent sessions, operator tools, browser control, and a rich operator system.", overview_slide),
        ...
        Slide("Agents and MPF", "Agents are data, not hard-coded behavior. The registry points to payloads and the payloads carry the operator.", agents_slide),
