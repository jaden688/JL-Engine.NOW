...
4. Routes all chat, run, and mission requests through that profile's operator, behavior rules, and backend settings.
...
| Payload file | `fat_agents/SparkByte_Full.json` (SparkByte operator) |
| Default backend | `ollama-local` |
| Drive type | fat-agent |
| Tags | safe, helper |
