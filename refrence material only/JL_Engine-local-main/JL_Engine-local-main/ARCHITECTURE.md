...
- `jl_engine_core` owns MPF registry loading, operator selection, behavior state, memory, and backend orchestration
...
The MPF registry is a pointer layer, not the full operator.
...
That means fat agents are not bypassing the engine. They are the operator capsules the engine actively loads and runs.
