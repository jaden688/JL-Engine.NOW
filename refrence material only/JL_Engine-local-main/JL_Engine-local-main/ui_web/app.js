...
function agentIdFromOperator(personaName) {
  return JL_FAT_AGENT_ID;
}

function setAgentIdInputFromOperator(personaName) {
  const input = $("agentId");
  if (!input) return;
  input.value = agentIdFromOperator(personaName);
}

function applyPrimaryProductSelection() {
  syncPersonaSelectors(PRIMARY_PRODUCT_SELECTION.operator);
  ...
  renderSelectionChips({
    ...
    agent_name: PRIMARY_PRODUCT_SELECTION.operator,
...
function renderAgents() {
  ...
  for (const agent of state.agents) {
    ...
    const agentProfile = agent.agent || agent.operator || "n/a";
  ...
  try {
    ...
    if (!state.activeAgentId && state.agents.length) {
      const preferred =
        state.agents.find(
          (item) =>
            normalizeToken(item.agent || item.operator || item.active_child || "") ===
            normalizeToken(PRIMARY_PRODUCT_SELECTION.operator),
        ) || state.agents[0];
      setActiveAgent(preferred.agent_id);
      syncPersonaSelectors(preferred.agent || preferred.operator || preferred.agent_id);
...
function renderMpfPersonaOptions(emptyLabel = "Loading operators...") {
  const operators = Array.isArray(state.mpfOperators) ? state.mpfOperators : [];
  ...
  for (const targetId of targets) {
    ...
    const options = operators
      .filter((operator) => operator.exists)
      .map((operator) => {
        const tags = Array.isArray(operator.tags) && operator.tags.length ? ` | ${operator.tags.join(", ")}` : "";
        return `<option value="${operator.name}">${operator.name}${tags}</option>`;
      });
    select.innerHTML = options.join("");
    if (prior && operators.some((operator) => operator.exists && operator.name === prior)) {
      select.value = prior;
      continue;
    }
    if (operators.some((operator) => operator.exists && operator.name === "SparkByte")) {
      select.value = "SparkByte";
      continue;
    }
    const first = operators.find((operator) => operator.exists);
    if (first) {
      select.value = first.name;
    }
  }
  const selectedOperator = $("chatPersonaSelect")?.value?.trim() || $("mpfPersonaSelect")?.value?.trim() || "SparkByte";
  setAgentIdInputFromOperator(selectedOperator);
}

async function loadMpfOperators(options = {}) {
  const { silent = false } = options;
  try {
    const data = await api("/quest/agents/profiles/mpf");
    const profiles = Array.isArray(data.agent_profiles)
      ? data.agent_profiles
      : (Array.isArray(data.agents) ? data.agents : data.operators);
    state.mpfOperators = Array.isArray(profiles) ? profiles : [];
    renderMpfPersonaOptions("No MPF agent profiles found");
    const existingCount = state.mpfOperators.filter((operator) => operator.exists).length;
...
function getPreferredMpfOperator() {
  ...
  const existing = (state.mpfOperators || []).find((operator) => operator.exists && operator.name === "SparkByte");
  if (existing) return existing.name;
  const fallback = (state.mpfOperators || []).find((operator) => operator.exists);
  return fallback ? fallback.name : "SparkByte";
}

function getDefaultMpfOperator() {
  const existing = (state.mpfOperators || []).find((operator) => operator.exists && operator.name === "SparkByte");
  if (existing) return existing.name;
  const fallback = (state.mpfOperators || []).find((operator) => operator.exists);
  return fallback ? fallback.name : "SparkByte";
...
function resolvePersonaByAlias(aliasKey) {
  const aliases = OPERATOR_PRESET_ALIASES[aliasKey] || [];
  const wanted = new Set(aliases.map(normalizeToken));
  if (!wanted.size) return null;
  const operators = (state.mpfOperators || []).filter((operator) => operator.exists);
  for (const operator of operators) {
    if (wanted.has(normalizeToken(operator.name))) {
...
function syncPersonaSelectors(personaName) {
  ...
  setAgentIdInputFromOperator(personaName);
...
async function activatePersonaByName(personaName) {
  ...
  if (switchboardSelection) {
    ...
    return {
      ...
      operator_name: String(switchboardSelection.entry?.agent_name || requested),
    };
  }
  const targetAgentId = agentIdFromOperator(requested);
  const res = await api("/quest/agents/register-mpf-operator", {
    method: "POST",
    body: JSON.stringify({
  ...
  syncPersonaSelectors(res.agent_name || res.operator_name || requested);
  await refreshAgents();
  await loadSwitchboard({ silent: true });
  await loadTools();
  feed(`MPF fat agent profile active: ${res.agent_name || res.operator_name || requested}`, "ok");
  return res;
}

async function activateChatOperator() {
  try {
    const selected = $("chatPersonaSelect")?.value?.trim() || getPreferredMpfOperator();
...
async function activatePresetOperator(aliasKey) {
  if (!state.mpfOperators.length) {
    await loadMpfOperators();
  }
...
async function ensureActiveAgentFromMpf(options = {}) {
  ...
  const personaName = forcePrimary ? PRIMARY_PRODUCT_SELECTION.operator : getDefaultMpfOperator();
  const targetAgentId = agentIdFromOperator(personaName);
  ...
  const res = await api("/quest/agents/register-mpf-operator", {
    ...
    body: JSON.stringify({
      ...
      operator_name: personaName,
    }),
  });
  setActiveAgent(res.agent?.agent_id || targetAgentId);
  syncPersonaSelectors(res.agent_name || res.operator_name || personaName);
  feed(`MPF-first bootstrap: ${targetAgentId} on ${res.agent_name || res.operator_name || personaName}`, "ok");
  await refreshAgents();
  return targetAgentId;
}
...
async function activateMpfAgent() {
  try {
    const selectedOperator = $("mpfPersonaSelect").value.trim();
    const targetOperator = selectedOperator || getPreferredMpfOperator();
    const agentId = agentIdFromOperator(targetOperator);
    const customMpfPath = $("mpfPath").value.trim();
    const switchboardSelection = !customMpfPath ? resolveSwitchboardSelectionByAgentName(targetOperator) : null;
    if (switchboardSelection) {
      if ($("laneSelect")) $("laneSelect").value = switchboardSelection.lane;
      renderSwitchboardChildren(switchboardSelection.child);
      await switchActiveLane();
    ...
    let endpoint = "/quest/agents/register-mpf-operator";
    let payload = {
      agent_id: agentId,
      agent_name: targetOperator,
      operator_name: targetOperator,
    };
    if (customMpfPath) {
    ...
    setActiveAgent(res.agent_id || agentId);
    if (res.agent_name || res.operator_name) {
      syncPersonaSelectors(res.agent_name || res.operator_name);
    } else if (payload.operator_name) {
      syncPersonaSelectors(payload.operator_name);
    }
    feed(`MPF agent profile active: ${res.agent_name || res.operator_name || payload.operator_name || "selected"}`, "ok");
    await refreshAgents();
    await loadSwitchboard({ silent: true });
    await loadTools();
  } catch (err) {
...
async function runMission() {
  ...
  const dynamicOperator = $("dynamicPersonaCheck")?.checked !== false;
  const selectedOperator = $("mpfPersonaSelect")?.value?.trim() || "";
  const personaOverride = dynamicOperator ? null : selectedOperator || getPreferredMpfOperator();
  ...
  try {
    ...
    const res = await api("/quest/mission", {
      ...
      body: JSON.stringify({
        ...
        task,
        dynamic_agent: dynamicOperator,
        agent: personaOverride,
        dynamic_operator: dynamicOperator,
        operator: personaOverride,
      }),
    });
    if (res.agent_id) {
      setActiveAgent(res.agent_id);
    ...
    setLatency(performance.now() - t0);
    const selected = res.selected_agent || res.selected_operator || "n/a";
...
function wireEvents() {
  ...
  bindEvent("activateChatPersonaBtn", "click", activateChatOperator);
  bindEvent("chatSparkByteBtn", "click", () => activatePresetOperator("sparkbyte"));
  bindEvent("chatGremlenBtn", "click", () => activatePresetOperator("gremlen"));
  bindEvent("chatSlappyBtn", "click", () => activatePresetOperator("slappy"));
  bindEvent("presetSparkByteBtn", "click", () => activatePresetOperator("sparkbyte"));
  bindEvent("presetGremlenBtn", "click", () => activatePresetOperator("gremlen"));
  bindEvent("presetSlappyBtn", "click", () => activatePresetOperator("slappy"));
  ...
  bindEvent("refreshMpfPersonasBtn", "click", loadMpfOperators);
...
async function boot() {
  ...
  const loadedOperators = await loadMpfOperators({ silent: true });
  if (!loadedOperators) {
